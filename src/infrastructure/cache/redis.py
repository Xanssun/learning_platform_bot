...
class RedisCache:
    ...
    async def setnx(
        ...
        return bool(await self._redis.set(key, value, ex=expire, nx=True))

    async def exists(self, key: str) -> bool:
        return bool(await self._redis.keys(key))

    async def keys(self, pattern: str | None = None) -> list[str]:
        return await self._redis.keys(pattern or "*")

    async def close(self) -> None:
        await self._redis.aclose(close_connection_pool=True)  # type: ignore


def get_redis_pool(settings: RedisSettings, **kw: Any) -> aioredis.Redis: # type: ignore
